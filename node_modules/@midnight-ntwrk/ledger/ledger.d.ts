...
export class ContractOperationVersionedVerifierKey {
  constructor(version: 'v1', rawVk: Uint8Array);
  readonly version: 'v1';
 ...
 *
 * In the next major version it will be removed in favor of `LocalStateNoKeys`
 */
export class LocalState {
   ...
   * recovery phrase)
   *
   * Use only for compatibility reasons - `fromSeedSpec` is the advised method to use for new wallets
   */
  static fromSeed(seed: Uint8Array): LocalState;

  /**
   * Creates a new state from a predefined random seed, deriving keys using the same algorithm as `SecretKeys::fromSeed`
   */
  static fromSeedSpec(seed: Uint8Array): LocalState;

  /**
   * Turns `LocalState` instance into a `LocalStateNoKeys` one, where keys are not held internally
   */
  removeKeys(): LocalStateNoKeys;

  /**
   * Allows to access secret keys, so that they can be stored separately from state
   */
  yesIKnowTheSecurityImplicationsOfThis_secretKeys(): SecretKeys;
 ...
 * Merkle tree to generate proofs against.
 *
 * Contrary to `LocalState`, it does not store keys internally, but accepts them as arguments to various operations.
 *
 * In the next major version of this library, it will replace the `LocalState`
 */
export class LocalStateNoKeys {
  /**
   ...
   * updates *are* included.
   */
  applyCollapsedUpdate(update: MerkleTreeCollapsedUpdate): LocalStateNoKeys;
  /**
   * Locally marks an offer as failed, allowing inputs used in it to be
   * spendable once more.
   */
  applyFailed(offer: Offer): LocalStateNoKeys;
  /**
   * Locally marks an proof-erased offer as failed, allowing inputs used in it
   * to be spendable once more.
   */
  applyFailedProofErased(offer: ProofErasedOffer): LocalStateNoKeys;
  /**
   * Locally applies an offer to the current state, returning the updated state
   */
  apply(secretKeys: SecretKeys, offer: Offer): LocalStateNoKeys;
  /**
   * Locally applies a proof-erased offer to the current state, returning the
   * updated state
   */
  applyProofErased(secretKeys: SecretKeys, offer: ProofErasedOffer): LocalStateNoKeys;
  /**
   * Locally applies a transaction to the current state, returning the updated
   * state
   *
   ...
   * ledger state
   */
  applyTx(secretKeys: SecretKeys, tx: Transaction, res: 'success' | 'partialSuccess' | 'failure'): LocalStateNoKeys;
  /**
   * Locally applies a proof-erased transaction to the current state, returning
   * the updated state
   *
   ...
   * ledger state
   */
  applyProofErasedTx(secretKeys: SecretKeys, tx: ProofErasedTransaction, res: 'success' | 'partialSuccess' | 'failure'): LocalStateNoKeys;
  /**
   * Locally applies a system transaction to the current state, returning the
   * updated state
   */
  applySystemTx(secretKeys: SecretKeys, tx: SystemTransaction): LocalStateNoKeys;
  /**
   * Initiates a new spend of a specific coin, outputting the corresponding
   * {@link UnprovenInput}, and the updated state marking this coin as
   * in-flight.
   */
  spend(secretKeys: SecretKeys, coin: QualifiedCoinInfo): [LocalStateNoKeys, UnprovenInput];
  /**
   * Initiates a new spend of a new-yet-received output, outputting the
   * corresponding {@link UnprovenTransient}, and the updated state marking
   * this coin as in-flight.
   */
  spendFromOutput(secretKeys: SecretKeys, coin: QualifiedCoinInfo, output: UnprovenOutput): [LocalStateNoKeys, UnprovenTransient];
  /**
   * Adds a coin to the list of coins that are expected to be received
   *
   * This should be used if an output is creating a coin for this wallet, which
   ...
   * know the commitment ahead of time to notice the receipt.
   */
  watchFor(coinPublicKey: CoinPublicKey, coin: CoinInfo): LocalStateNoKeys;
  serialize(netid: NetworkId): Uint8Array;
  static deserialize(raw: Uint8Array, netid: NetworkId): LocalStateNoKeys;
  toString(compact?: boolean): string;
  /**
   * The coin public key of this wallet
   */
...
export class UnprovenInput {
   ...
   * contract
   */
  static newContractOwned(coin: QualifiedCoinInfo, contract: ContractAddress, state: ZswapChainState): UnprovenInput;
  serialize(netid: NetworkId): Uint8Array;
...
export class UnprovenOutput {
   ...
   */
  static new(coin: CoinInfo, target_cpk: CoinPublicKey, target_epk?: EncPublicKey): UnprovenOutput;
  /**
   ...
   */
  static newContractOwned(coin: CoinInfo, contract: ContractAddress): UnprovenOutput;
  serialize(netid: NetworkId): Uint8Array;
...
export class UnprovenTransient {
   ...
   */
  static newFromContractOwnedOutput(coin: QualifiedCoinInfo, output: UnprovenOutput): UnprovenTransient;
  serialize(netid: NetworkId): Uint8Array;
